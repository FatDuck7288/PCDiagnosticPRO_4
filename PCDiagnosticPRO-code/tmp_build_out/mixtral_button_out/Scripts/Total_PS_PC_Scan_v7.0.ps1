...
# corrupts French accented characters (é → Ã©, è → Ã¨, etc.)
# =============================================================================
...
try {
    ...
    # This affects native commands and legacy behavior
    $null = chcp 65001 2>&1
    # If chcp fails (e.g., in non-interactive mode), continue anyway
    # The [Console]::OutputEncoding is the most important one
}
catch {
    ...
    try {
        ...
        for ($i = 0; $i -lt $wmiCount; $i++) {
            ...
            $gpuInfo = [ordered]@{
                ...
                status        = $status
                vramTotalMB   = $vramMB
                vramNote      = if ($vramAvailable) { "$vramMB MB [$vramSrc]" } else { 'Non disponible (limitation WMI - collecte externalisee)' }
                vramInfo      = $vramInfo
            }
            $gpuList += $gpuInfo
