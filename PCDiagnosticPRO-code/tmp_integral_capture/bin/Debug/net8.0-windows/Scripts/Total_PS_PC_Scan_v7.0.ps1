...
#region [LICENSE]
# Ce script intÃ¨gre la bibliothÃ¨que **LibreHardwareMonitorLib.dll** distribuÃ©e sous licence MPLÂ 2.0.
# La licence MPLÂ 2.0 est disponible Ã  l'adresseÂ : https://www.mozilla.org/MPL/2.0/.
# ConformÃ©ment aux exigences de licence, cette sonde de tempÃ©rature est identifiÃ©e comme le Â«Â Module de tempÃ©rature Virtual IT ProÂ Â»
...
param(
    ...
    [switch]$QuickScan,
    [ValidateRange(3, 60)]
    # OPTIMISATION: RÃ©duit de 10s Ã  3s (suffisant pour moyennes CPU/Disk)
...
#
# Helper: exÃ©cute une commande avec timeout et capture stdout/stderr
#
...
#
# Convertit rÃ©cursivement un objet en structure sÃ©rialisable JSON (pas d'OrderedDictionary, clÃ©s string)
# - Les dictionnaires sont transformÃ©s en hashtable Ã  clÃ©s string
# - Les listes/Ã©numÃ©rables sont converties en tableaux de valeurs sÃ»res
# - Les PSCustomObject et objets .NET avec propriÃ©tÃ©s sont convertis en hashtable de propriÃ©tÃ©s
# - Les chaÃ®nes et types primitifs sont retournÃ©s tels quels
# Ce helper garantit que ConvertTo-Json ne lÃ¨ve pas d'exception sous PowerShellÂ 5.1.
function ConvertTo-JsonSafeObject {
    param([object]$InputObject)
    # Valeur nulle
    if ($null -eq $InputObject) { return $null }
    # Ne pas traiter les chaÃ®nes comme IEnumerable
    if ($InputObject -is [string] -or $InputObject -is [char] -or $InputObject.GetType().IsPrimitive) {
        return $InputObject
    }
    # Dictionnaires: hashtable Ã  clÃ©s string
    ...
    # PSCustomObject ou objets .NET: convertir propriÃ©tÃ©s en hashtable
    ...
    # Fallback: retourner sous forme de chaÃ®ne
...
#
# Helper: retourne le temps CPU en secondes pour un processus, en gÃ©rant tous les types sans lever d'exception
#
function Get-ProcCpuSeconds {
    param([object]$Proc)
    try {
        if ($null -eq $Proc) { return 0.0 }
        # Obtenir la valeur CPU de faÃ§on sÃ»re
        $cpuVal = Get-SafePropValue $Proc 'CPU'
        # Si la propriÃ©tÃ© CPU est renseignÃ©e, tenter de calculer les secondes CPU
        if ($null -ne $cpuVal) {
            ...
            # Si la propriÃ©tÃ© TotalSeconds existe, l'utiliser (ex: TimeSpan masquÃ©)
            $matchCount = 0
...
# Retourne $true si la valeur est jugÃ©e fiable, $false sinon
function Test-VramValue {
    ...
    try {
        # Si VRAM est absent ou infÃ©rieure Ã  512Â MB, invalide
        if ($null -eq $vramMB -or $vramMB -lt 512) { return $false }
        # Doit Ãªtre un multiple de 256Â MB
        if (($vramMB % 256) -ne 0) { return $false }
        # Rejeter certaines valeurs erronÃ©es frÃ©quentes
        if ($vramMB -in @(4095, 2047, 1023)) { return $false }
        $lower = ''
        if ($null -ne $gpuName) { $lower = $gpuName.ToLower() }
        # Cartes haut de gamme : minimum 8Â GB (8192Â MB)
...
#
# Helper: rÃ©cupÃ¨re les informations GPU via nvidia-smi si disponible
...
function Get-NvidiaGpuInfo {
    try {
        ...
        if ($null -ne $cmd) {
            $nvidiaArgs = @('--query-gpu=index,name,memory.total,driver_version', '--format=csv,noheader,nounits')
...
#
# Helper: rÃ©cupÃ¨re les informations GPU via dxdiag
# Utilise l'export texte de dxdiag (/t) et analyse les lignes pour obtenir le nom et la mÃ©moire dÃ©diÃ©e
function Get-DxdiagGpuInfo {
    try {
        $list = @()
        $tempFile = [System.IO.Path]::Combine($env:TEMP, "dxdiag_gpu_$([Guid]::NewGuid().ToString()).txt")
        # ExÃ©cuter dxdiag en mode silencieux vers fichier texte
        ...
        if (Test-Path $tempFile) {
            ...
            foreach ($ln in $lines) {
                ...
                # Trouver la mÃ©moire dÃ©diÃ©e (Dedicated Memory ou Dedicated Video Memory)
...
# Helper: charge la DLL LibreHardwareMonitorLib.dll si elle est prÃ©sente (avec tÃ©lÃ©chargement optionnel) et retourne $true/$false
...
function Import-LibreHardwareMonitor {
    ...
    .DESCRIPTION
        Recherche dans plusieurs emplacements: app packagÃ©e, script, Program Files.
        LICENCE: LibreHardwareMonitor est sous MPL 2.0 - Ã  vÃ©rifier par l'Ã©quipe.
    #>
    try {
        ...
        # Chemins possibles (app packagÃ©e prioritaire)
        $possiblePaths = @(
...
function Show-Progress {
    ...
    # Legacy wrapper â€” delegates to Write-Status for structured output
...
function Get-ScoreV2 {
    ...
    .DESCRIPTION
        ...
        - CRITICAL/FATAL: -25 (max 2 comptÃ©s = -50 cap)
        - COLLECTOR_ERROR: -3 par section (sauf limitations techniques)
        - WARN: -1 (max 10 comptÃ©s)
        - TIMEOUT: -5
    ...
    $totalPenalty = 0
    $totalPenalty += [math]::Min($penalties.critical, 2) * 25  # Max 2 critiques comptÃ©s
    $totalPenalty += $penalties.collectorErrors * 3
    $totalPenalty += [math]::Min($penalties.warnings, 10) * 1  # Max 10 warns comptÃ©s
    $totalPenalty += $penalties.timeouts * 5
    # INFO et limitations techniques ne penalisent pas le score
    
    # Score minimum 10 (sauf si 0 sections collectÃ©es)
...
function Get-JsonSnapshotString {
    ...
    return (ConvertTo-JsonSafeObject $snapshot) | ConvertTo-Json -Depth 10 -Compress
}
...
function Get-GpuInfo {
    <#
    .SYNOPSIS
        Collecte GPU v7.0 - DonnÃ©es fiables uniquement
    .DESCRIPTION
        VRAM et tempÃ©rature GPU neutralisÃ©es (limitation WMI - collecte externalisÃ©e)
        Conserve: Nom, Fabricant, Driver, Date, RÃ©solution, PNPDeviceID
    #>
    $content = @()
    $data = [ordered]@{}
    try {
        ...
        for ($i = 0; $i -lt $wmiCount; $i++) {
            ...
            # v7.0: VRAM neutralisÃ©e (limitation WMI)
            $gpuInfo = [ordered]@{
                name = $name
                vendor = $vendor
...
#
# Collecte des tempÃ©ratures CPU/GPU/Disques
# v7.0: CPU et GPU neutralisÃ©s (limitation WMI - collecte externalisÃ©e)
# Seules les tempÃ©ratures disques sont conservÃ©es si disponibles
function Get-Temperatures {
    <#
    .SYNOPSIS
        Collecte tempÃ©ratures v7.0 - DonnÃ©es fiables uniquement
    .DESCRIPTION
        CPU et GPU: NeutralisÃ©s (limitation WMI - collecte externalisÃ©e vers Brique 2)
        Disques: ConservÃ©s via StorageReliabilityCounter/SMART si disponibles
    #>
    $content = @()
    $data = [ordered]@{}
    $degC = "$([char]176)C"
    ...
    try {
        ...
        # TempÃ©ratures disques (conservÃ©es)
        ...
        # DonnÃ©es JSON (cpuTempC alimente l'UI TempÃ©rature CPU quand disponible)
...
function Get-WindowsUpdateInfo {
    ...
    try {
        ...
        # OPTIMISATION: Cache - Skip recherche complÃ¨te si derniÃ¨re vÃ©rification < 24h
        try {
...
function Get-SmartDetails {
    ...
    try {
        ...
        # OPTIMISATION: RÃ©cupÃ©rer liste des disques USB Ã  ignorer
        $usbDiskPatterns = @()
        try {
            $physicalDisks = Get-PhysicalDisk -ErrorAction SilentlyContinue
            if ($physicalDisks) {
                foreach ($pd in $physicalDisks) {
                    $busType = $pd.BusType
                    # Skip USB, Removable, et trÃ¨s petits disques (< 10GB = probablement clÃ© USB)
        ...
        if ($null -ne $smartStatus) {
            ...
            foreach ($status in $statusArray) {
                ...
                # OPTIMISATION: Skip disques USB/removable identifiÃ©s
                $skipDisk = $false
            ...
            # Log des disques ignorÃ©s (USB/removable)
...
function Get-DynamicSignals {
    ...
    try {
        ...
        $topCpu = @(); $topMemory = @()
        try {
            # Tri sÃ»r des processus par consommation CPU (en secondes) via Get-ProcCpuSeconds
            ...
            # Tri des processus par consommation mÃ©moire (WorkingSet64) inchangÃ©
            $procsM = $allProcs |
...
try {
    ...
    $collectors = @(
        ...
        @{Name='Registre'; Function={Get-RegistryKeysInfo}; Section='27'}
        @{Name='Temperatures'; Function={Get-Temperatures}; Section='28'}
        @{Name='Integrite Systeme'; Function={Get-SystemIntegrity}; Section='29'}
        @{Name='Alimentation'; Function={Get-PowerSettings}; Section='30'}
        @{Name='Analyse Minidumps'; Function={Get-MinidumpAnalysis}; Section='31'}
        @{Name='Historique Fiabilite'; Function={Get-ReliabilityHistory}; Section='32'}
        @{Name='Compteurs Performance'; Function={Get-PerformanceCounters}; Section='33'}
        @{Name='Latence Reseau'; Function={Get-NetworkInfoLatency}; Section='34'}
        @{Name='SMART Detaille'; Function={Get-SmartDetails}; Section='35'}
    )
