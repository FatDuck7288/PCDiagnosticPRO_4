...
namespace PCDiagnosticPro.DiagnosticsSignals
{
    ...
    public class SignalsOrchestrator
    {
        ...
        private readonly SemaphoreSlim _cpuBoundGate = new(2, 2);
        private readonly SemaphoreSlim _ioBoundGate = new(2, 2);
        ...
        public async Task<DiagnosticSignalsResult> CollectAllAsync(CancellationToken ct = default)
        {
            ...
            // Run collectors in bounded parallelism to avoid CPU/network/IO contention.
            var tasks = _collectors.Select(collector => RunCollectorWithResourceGateAsync(collector, ct)).ToList();
        ...
        private enum CollectorResourceClass
        {
            CpuBound,
            IoBound,
            Network
        ...
        private async Task<SignalResult> RunCollectorWithResourceGateAsync(ISignalCollector collector, CancellationToken ct)
        {
            var resourceClass = GetResourceClass(collector.Name);
            var gate = GetGate(resourceClass);
            var acquired = false;
            ...
            try
            {
                ...
                acquired = true;
                return await RunCollectorAsync(collector, ct).ConfigureAwait(false);
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested)
            {
                return SignalResult.Unavailable(collector.Name, "cancelled", "collector");
            }
            finally
            {
                if (acquired)
        ...
        private CollectorResourceClass GetResourceClass(string collectorName)
        {
            if (string.IsNullOrWhiteSpace(collectorName))
                return CollectorResourceClass.CpuBound;

            if (collectorName.Contains("network", StringComparison.OrdinalIgnoreCase) ||
                collectorName.Contains("internetSpeed", StringComparison.OrdinalIgnoreCase))
            {
                return CollectorResourceClass.Network;
            }

            if (collectorName.Contains("io", StringComparison.OrdinalIgnoreCase) ||
                collectorName.Contains("driver", StringComparison.OrdinalIgnoreCase) ||
                collectorName.Contains("boot", StringComparison.OrdinalIgnoreCase))
            {
                return CollectorResourceClass.IoBound;
            }

            return CollectorResourceClass.CpuBound;
        }

        private SemaphoreSlim GetGate(CollectorResourceClass resourceClass)
        {
            return resourceClass switch
            {
                CollectorResourceClass.Network => _networkGate,
                CollectorResourceClass.IoBound => _ioBoundGate,
                _ => _cpuBoundGate
            };
        }
