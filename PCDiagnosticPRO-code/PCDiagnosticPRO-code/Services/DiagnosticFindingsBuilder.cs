...
namespace PCDiagnosticPro.Services
{
    ...
    /// Safety Gate AutoFix is score-agnostic: problem detection drives availability.
    ...
    public static class DiagnosticFindingsBuilder
    {
        ...
        /// <summary>
        /// Safety Gate AutoFix (score-agnostic): availability follows detected problems only.
        /// </summary>
        public static (bool Allowed, string? BlockReason) EvaluateSafetyGate(
            ...
            JsonElement? psRoot)
        {
            var (problemDetected, _) = DetectProblemSignals(findings, report, psRoot);
            if (!problemDetected)
            {
                return (false, "Aucun probleme detecte.");
            }

            return (true, null);
        }

        public static (bool ProblemDetected, string Summary) DetectProblemSignals(
            IReadOnlyList<DiagnosticFinding> findings,
            HealthReport report,
            JsonElement? psRoot)
        {
            int normalizedFindings = findings?.Count ?? 0;
            int reportErrors = report?.Errors?.Count ?? 0;
            int explicitFindings = 0;
            int snapshotFindings = 0;
            int snapshotErrors = 0;
            int criticalEvents = 0;
            int unstableSignals = 0;

            if (psRoot.HasValue && psRoot.Value.ValueKind == JsonValueKind.Object)
            {
                var root = psRoot.Value;
                explicitFindings = Math.Max(explicitFindings, GetArrayCount(root, "findings"));
                snapshotFindings = Math.Max(snapshotFindings, GetArrayCount(root, "diagnostic_snapshot", "findings"));
                snapshotErrors = Math.Max(snapshotErrors, GetArrayCount(root, "diagnostic_snapshot", "collectionQuality", "errors"));
                reportErrors = Math.Max(reportErrors, GetArrayCount(root, "errors"));
                criticalEvents = Math.Max(criticalEvents, CountCriticalOrErrorEvents(root));
                unstableSignals = Math.Max(unstableSignals, CountUnstableSignals(root));

                if (root.TryGetProperty("scan_powershell", out var scanPs) && scanPs.ValueKind == JsonValueKind.Object)
                {
                    explicitFindings = Math.Max(explicitFindings, GetArrayCount(scanPs, "findings"));
                    reportErrors = Math.Max(reportErrors, GetArrayCount(scanPs, "errors"));
                }
            }
            ...
            var summary = $"findings={normalizedFindings + explicitFindings + snapshotFindings}, " +
                          $"errors={reportErrors + snapshotErrors}, " +
                          $"events={criticalEvents}, signals={unstableSignals}";

            return (problemDetected, summary);
        }
        ...
        private static int CountCriticalOrErrorEvents(JsonElement root)
        {
            if (root.ValueKind != JsonValueKind.Object
                || !root.TryGetProperty("event_logs_detailed", out var events)
            ...
            {
                if (ev.ValueKind != JsonValueKind.Object || !ev.TryGetProperty("level", out var level))
                {
                    continue;
                }

                if ((level.ValueKind == JsonValueKind.Number && (level.GetInt32() == 1 || level.GetInt32() == 2))
                    || (level.ValueKind == JsonValueKind.String
                        && (string.Equals(level.GetString(), "critical", StringComparison.OrdinalIgnoreCase)
        ...
        private static int CountUnstableSignals(JsonElement root)
        {
            if (root.ValueKind != JsonValueKind.Object
                || !root.TryGetProperty("diagnostic_signals", out var signals)
            ...
            {
                var signal = signalProperty.Value;
                ...
                bool available = true;
                if (signal.TryGetProperty("available", out var availableEl) && availableEl.ValueKind == JsonValueKind.False)
                {
                    available = false;
                }

                string quality = string.Empty;
                if (signal.TryGetProperty("quality", out var qualityEl) && qualityEl.ValueKind == JsonValueKind.String)
                {
                    quality = qualityEl.GetString() ?? string.Empty;
                }

                if (!available
                    || quality.Equals("suspect", StringComparison.OrdinalIgnoreCase)
