...
namespace PCDiagnosticPro.Services
{
    ...
    /// Schema 2.3.0: Maps PS sections + C# collectors to normalized snapshot.
    ...
    public class DiagnosticSnapshotBuilder
    {
        ...
        public DiagnosticSnapshotBuilder()
        {
            ...
            LogBuild($"[DiagnosticSnapshotBuilder] Initialized (schemaVersion={PCDiagnosticPro.Models.SchemaRegistry.SnapshotSchemaVersion})");
        }

        /// <summary>
        /// Add CPU metrics from HardwareSensorsResult (C# sensors).
        /// MERGE POLICY: C# sensors (LibreHardwareMonitor) are the authoritative source
        /// for CPU/GPU temperatures. The PS script also collects temperatures (WMI/nvidia-smi)
        /// but those go to the raw JSON only — AddPowerShellData intentionally does NOT map
        /// the Temperatures section to avoid conflicts. If C# sensors are unavailable,
        /// consumers should fall back to PS Temperatures section in the raw JSON.
        /// </summary>
        public DiagnosticSnapshotBuilder AddCpuMetrics(HardwareSensorsResult? sensors)
        {
            ...
            var collectorSource = sensors?.SafeModeUsed == true
                ? "SafeHardwareSensorsCollector"
                : "HardwareSensorsCollector";
            ...
            {
                cpuMetrics["temperature"] = MetricFactory.CreateUnavailable("°C", collectorSource, "sensor_not_available");
            }
        ...
        public DiagnosticSnapshotBuilder AddGpuMetrics(HardwareSensorsResult? sensors)
        {
            ...
            if (sensors?.Gpu != null)
            {
                gpuMetrics["name"] = sensors.Gpu.Name.Available
                    ? MetricFactory.CreateAvailable(sensors.Gpu.Name.Value ?? "", "", collectorSource, 100)
                    : MetricFactory.CreateUnavailable("", collectorSource, sensors.Gpu.Name.Reason ?? "not_detected");

                var gpuTempSource = string.IsNullOrWhiteSpace(sensors.Gpu.GpuTempSource)
                    ? collectorSource
                    : sensors.Gpu.GpuTempSource;
                var gpuLoadSource = sensors.SafeModeUsed
                    ? "PerformanceCounter GPU Engine (engtype_3D)"
                    : "LibreHardwareMonitor GPU Core";
                var vramTotalSource = collectorSource;
                var vramUsedSource = string.IsNullOrWhiteSpace(sensors.Gpu.VramUsedSource)
                    ? collectorSource
                    : sensors.Gpu.VramUsedSource;

                gpuMetrics["temperature"] = NormalizeGpuTemp(sensors.Gpu.GpuTempC, gpuTempSource);
                gpuMetrics["load"] = NormalizePercent(sensors.Gpu.GpuLoadPercent, gpuLoadSource, "gpu_load");
                gpuMetrics["vramTotalMB"] = NormalizeVramTotal(sensors.Gpu.VramTotalMB, vramTotalSource);
                gpuMetrics["vramUsedMB"] = NormalizeVramUsed(sensors.Gpu.VramUsedMB, sensors.Gpu.VramTotalMB, vramUsedSource);
            }
            else
            {
                gpuMetrics["temperature"] = MetricFactory.CreateUnavailable("°C", collectorSource, "gpu_not_detected");
                gpuMetrics["load"] = MetricFactory.CreateUnavailable("%", collectorSource, "gpu_not_detected");
            }
        ...
        public DiagnosticSnapshotBuilder AddStorageMetrics(HardwareSensorsResult? sensors)
        {
            ...
            if (sensors?.Disks != null && sensors.Disks.Count > 0)
            {
                ...
                foreach (var disk in sensors.Disks)
                {
                    ...
                    var tempMetric = NormalizeDiskTemp(disk.TempC, diskName, collectorSource);
                ...
                storageMetrics["diskCount"] = MetricFactory.FromCount(sensors.Disks.Count, collectorSource);
            ...
            {
                storageMetrics["diskCount"] = MetricFactory.FromCount(0, collectorSource, "no_disks_detected");
            }
        ...
        #region PS Section Mapping (Schema 2.3.0)
        
        // ═══════════════════════════════════════════════════════════════
        // CONTRAT DE MAPPING PS → SNAPSHOT (schemaVersion 2.3.0)
        // ═══════════════════════════════════════════════════════════════
        //
        ...
        {
            var actualSource = string.IsNullOrWhiteSpace(source) ? "HardwareSensorsCollector" : source;
            var confidence = ResolveConfidenceFromSource(actualSource, metric?.Available == true);

            if (metric == null || !metric.Available)
                return MetricFactory.CreateUnavailable("°C", actualSource, metric?.Reason ?? "sensor_not_available");
            ...
            if (value < 5 || value > 115)
                return MetricFactory.CreateUnavailable("°C", actualSource, $"out_of_range ({value:F1}°C not in [5,115])");

            return MetricFactory.CreateAvailable(Math.Round(value, 1), "°C", actualSource, confidence);
        }

        private NormalizedMetric NormalizeGpuTemp(MetricValue<double>? metric, string source)
        {
            var actualSource = string.IsNullOrWhiteSpace(source) ? "HardwareSensorsCollector" : source;
            var confidence = ResolveConfidenceFromSource(actualSource, metric?.Available == true);
            ...
            if (value < 5 || value > 120)
                return MetricFactory.CreateUnavailable("°C", actualSource, $"out_of_range ({value:F1}°C not in [5,120])");

            return MetricFactory.CreateAvailable(Math.Round(value, 1), "°C", actualSource, confidence);
        }

        private NormalizedMetric NormalizeDiskTemp(MetricValue<double>? metric, string diskName, string source)
        {
            var actualSource = string.IsNullOrWhiteSpace(source) ? "HardwareSensorsCollector" : source;
            var confidence = ResolveConfidenceFromSource(actualSource, metric?.Available == true);

            if (metric == null || !metric.Available)
                return MetricFactory.CreateUnavailable("°C", actualSource, metric?.Reason ?? "disk_temp_not_available", diskName);

            var value = metric.Value;

            if (double.IsNaN(value) || double.IsInfinity(value))
                return MetricFactory.CreateUnavailable("°C", actualSource, "nan_or_infinite", diskName);

            if (Math.Abs(value) < 0.001)
                return MetricFactory.CreateUnavailable("°C", actualSource, "sentinel_zero", diskName);

            if (value < 0 || value > 90)
                return MetricFactory.CreateUnavailable("°C", actualSource, $"out_of_range ({value:F1}°C not in [0,90])", diskName);

            return MetricFactory.CreateAvailable(Math.Round(value, 1), "°C", actualSource, confidence, diskName);
        }
        ...
        private NormalizedMetric NormalizePercent(MetricValue<double>? metric, string source, string name)
        {
            ...
            if (metric == null || !metric.Available)
                return MetricFactory.CreateUnavailable("%", actualSource, metric?.Reason ?? "not_available");

            var value = metric.Value;

            if (double.IsNaN(value) || double.IsInfinity(value))
                return MetricFactory.CreateUnavailable("%", actualSource, "nan_or_infinite");

            if (value < 0 || value > 100)
                return MetricFactory.CreateUnavailable("%", actualSource, $"out_of_range ({value:F1}% not in [0,100])");

            return MetricFactory.CreateAvailable(Math.Round(value, 1), "%", actualSource, confidence);
        }

        private NormalizedMetric NormalizeVramTotal(MetricValue<double>? metric, string source)
        {
            var actualSource = string.IsNullOrWhiteSpace(source) ? "HardwareSensorsCollector" : source;
            var confidence = ResolveConfidenceFromSource(actualSource, metric?.Available == true);

            if (metric == null || !metric.Available)
                return MetricFactory.CreateUnavailable("MB", actualSource, metric?.Reason ?? "not_available");

            var value = metric.Value;

            if (value <= 0)
                return MetricFactory.CreateUnavailable("MB", actualSource, "sentinel_zero_or_negative");

            return MetricFactory.CreateAvailable(Math.Round(value, 0), "MB", actualSource, confidence);
        }

        private NormalizedMetric NormalizeVramUsed(MetricValue<double>? usedMetric, MetricValue<double>? totalMetric, string source)
        {
            var actualSource = string.IsNullOrWhiteSpace(source) ? "HardwareSensorsCollector" : source;
            var confidence = ResolveConfidenceFromSource(actualSource, usedMetric?.Available == true);

            if (usedMetric == null || !usedMetric.Available)
                return MetricFactory.CreateUnavailable("MB", actualSource, usedMetric?.Reason ?? "not_available");

            var used = usedMetric.Value;
            var total = totalMetric?.Value ?? 0;

            if (used < 0)
                return MetricFactory.CreateUnavailable("MB", actualSource, "negative_value");

            if (total > 0 && used > total * 1.1)
                return MetricFactory.CreateUnavailable("MB", actualSource, $"vram_used_exceeds_total ({used:F0} > {total:F0})");

            return MetricFactory.CreateAvailable(Math.Round(used, 1), "MB", actualSource, confidence);
        }

        private static int ResolveConfidenceFromSource(string? source, bool available)
        {
            if (!available)
                return 0;

            var s = (source ?? string.Empty).ToLowerInvariant();
            if (s.Contains("librehardwaremonitor") || s.Contains("lhm") || s.Contains("nvml"))
                return 90;
            if (s.Contains("performancecounter") || s.Contains("perf"))
                return 80;
            if (s.Contains("wmi"))
                return 55;
            return 70;
