    ...
    public class ProcessTelemetryCollector
    {
        ...
        public async Task<ProcessTelemetryResult> CollectAsync(CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                var processes = await CollectViaSystemDiagnosticsWithCpuAsync(ct).ConfigureAwait(false);
                ...
                if (processes.Count == 0)
                {
                    ...
                    processes = await CollectViaToolhelp32WithCpuAsync(ct).ConfigureAwait(false);
                ...
                if (processes.Count == 0)
                {
                    ...
                    processes = await Task.Run(() => CollectViaWmiProcess(ct), ct).ConfigureAwait(false);
                ...
                if (processes.Count == 0)
                {
                    ...
                    processes = await Task.Run(() => CollectViaTasklist(ct), ct).ConfigureAwait(false);
        ...
        private async Task<List<ProcessInfo>> CollectViaSystemDiagnosticsWithCpuAsync(CancellationToken ct)
        {
            ...
            await Task.Delay(CpuSampleWindowMs, ct).ConfigureAwait(false);
        ...
        private async Task<List<ProcessInfo>> CollectViaToolhelp32WithCpuAsync(CancellationToken ct)
        {
            var result = new List<ProcessInfo>();
            var logicalCores = Environment.ProcessorCount;
