    ...
    public static class PerformanceDatasetLoader
    {
        ...
        private static void SaveCache(PerformanceDataset dataset, string? etag)
        {
            try
            {
                ...
                File.WriteAllText(CachePath, JsonSerializer.Serialize(dataset, options), Encoding.UTF8);
        ...
        private static void SaveMeta(CacheMeta meta)
        {
            try
            {
                ...
                File.WriteAllText(MetaPath, JsonSerializer.Serialize(meta, options), Encoding.UTF8);
            }
            catch (Exception ex)
            {
