...
namespace PCDiagnosticPro.Services
{
    ...
    public static class UiCompletenessValidator
    {
        ...
        private static readonly Dictionary<HealthDomain, (string[] requiredFields, int minFields)> Contracts = new()
        {
            ...
            { HealthDomain.PlatformFirmware, (new[] { "Version BIOS", "TPM", "Secure Boot" }, 3) },
        ...
        {
            var result = new ValidationResult { TotalSections = Enum.GetValues<HealthDomain>().Length };
            ...
            SectionValidation validation)
        {
            switch (domain)
            {
                ...
                case HealthDomain.PlatformFirmware:
                    if (validation.DataExistsInJson)
                    {
                        var hasBios = extraction.Evidence.ContainsKey("Version BIOS");
                        var hasTpm = extraction.Evidence.ContainsKey("TPM");
                        var hasSecureBoot = extraction.Evidence.ContainsKey("Secure Boot");
                        if (!hasBios) validation.Warnings.Add("MachineIdentity existe mais 'Version BIOS' non affiché");
                        if (!hasTpm) validation.Warnings.Add("MachineIdentity existe mais 'TPM' non affiché");
                        if (!hasSecureBoot) validation.Warnings.Add("MachineIdentity existe mais 'Secure Boot' non affiché");
                    }
                    break;
            }
        }
        ...
        private static bool CheckJsonDataExists(HealthDomain domain, JsonElement root)
        {
            var sectionName = domain switch
            {
                ...
                HealthDomain.PlatformFirmware => "MachineIdentity",
