...
namespace PCDiagnosticPro.Services
{
    ...
    public static class CollectorDiagnosticsService
    {
        ...
        public class CollectorDiagnosticsResult
        {
            ...
            /// <summary>Données manquantes normalisées (array de strings) — compat legacy</summary>
            public List<string> MissingDataNormalized { get; set; } = new();

            /// <summary>Données manquantes structurées v2 — préserve reason/source/confidence/timestamp</summary>
        ...
        public static CollectorDiagnosticsResult Analyze(JsonElement root, HardwareSensorsResult? sensors)
        {
            ...
            result.MissingDataStructured = ExtractMissingDataStructured(root);
            ...
            result.CollectorErrorsLogical = CalculateCollectorErrorsLogical(result, sensors);
        ...
        /// FIX2: Ne pas compter les erreurs PS de température/capteurs si le fallback C# a réussi
        ///        (sensors.Cpu.CpuTempC.Available == true). Évite de pénaliser un scan normal où
        ///        les capteurs WMI PS ont échoué mais LibreHardwareMonitor C# a compensé.
        /// </summary>
        private static int CalculateCollectorErrorsLogical(CollectorDiagnosticsResult result, HardwareSensorsResult? sensors)
        {
            bool csharpTempAvailable = sensors?.Cpu?.CpuTempC?.Available == true;

            int count = 0;
            foreach (var error in result.Errors)
            {
                if (csharpTempAvailable && IsTempSensorError(error))
                {
                    App.LogMessage($"[CollectorDiagnostics] Erreur température PS ignorée (fallback C# OK): code={error.Code} section={error.Section}");
                    continue;
                }
                count++;
            }

            App.LogMessage($"[CollectorDiagnostics] CollectorErrorsLogical={count} (errors[]={result.Errors.Count}, ignorées_temp={result.Errors.Count - count}, invalidatedMetrics={result.InvalidatedMetrics.Count} NON comptées)");
            return count;
        }

        private static bool IsTempSensorError(ScanErrorInfo error)
        {
            var code = error.Code ?? string.Empty;
            var section = error.Section ?? string.Empty;
            var message = error.Message ?? string.Empty;

            return ContainsTempKeyword(code) || ContainsTempKeyword(section) || ContainsTempKeyword(message);
        }

        private static bool ContainsTempKeyword(string value)
        {
            if (string.IsNullOrEmpty(value))
                return false;

            return value.IndexOf("temp", StringComparison.OrdinalIgnoreCase) >= 0
                || value.IndexOf("sensor", StringComparison.OrdinalIgnoreCase) >= 0
                || value.IndexOf("hardware", StringComparison.OrdinalIgnoreCase) >= 0
                || value.IndexOf("thermal", StringComparison.OrdinalIgnoreCase) >= 0
                || value.IndexOf("thermique", StringComparison.OrdinalIgnoreCase) >= 0
                || value.IndexOf("capteur", StringComparison.OrdinalIgnoreCase) >= 0;
        }
        ...
        /// <summary>
        /// Extrait missingData en conservant la structure complète (section/item/reason/source/confidence/timestamp).
        /// Supporte le nouveau format PS v2 avec source+confidence.
        /// Compatibilité : si l'entrée est une string, crée un objet avec source="legacy".
        /// </summary>
        private static List<MissingDataEntry> ExtractMissingDataStructured(JsonElement root)
        {
            var result = new List<MissingDataEntry>();
            if (root.ValueKind != JsonValueKind.Object) return result;
            if (!root.TryGetProperty("missingData", out var mdElement)) return result;

            if (mdElement.ValueKind == JsonValueKind.Array)
            {
                foreach (var item in mdElement.EnumerateArray())
                {
                    if (item.ValueKind == JsonValueKind.String)
                    {
                        result.Add(new MissingDataEntry
                        {
                            Item = item.GetString() ?? "", Source = "legacy", Confidence = "low",
        ...
        public static int ApplyConfidenceGating(int baseConfidence, CollectorDiagnosticsResult diagnostics)
        {
            ...
            // Gate 2: Données manquantes critiques — vérifier dans les entrées structurées si disponibles
            int criticalMissing;
            if (diagnostics.MissingDataStructured.Count > 0)
            {
                criticalMissing = diagnostics.MissingDataStructured.Count(e =>
                {
                    var combined = $"{e.Section} {e.Item} {e.Reason}";
                    return combined.Contains("CPU",    StringComparison.OrdinalIgnoreCase) ||
                           combined.Contains("GPU",    StringComparison.OrdinalIgnoreCase) ||
                           combined.Contains("Memory", StringComparison.OrdinalIgnoreCase) ||
                           combined.Contains("Disk",   StringComparison.OrdinalIgnoreCase);
                });
            }
            else
            {
                criticalMissing = diagnostics.MissingDataNormalized.Count(m =>
